...
> Status: **M43 complete.** **Content Browser polish.** The asset browser navigates like a real editor now. The
> path is a **clickable breadcrumb** — a ⌂ home crumb plus one button per folder, so you can jump up any number
> of levels in a click instead of tapping ↑ Up repeatedly. A **type-filter dropdown** (All / Maps / Bins /
> Textures / Meshes / Skeletons / Animations / Materials / Shaders / Audio) narrows the current folder to one
> asset kind while keeping subfolders visible so you can still drill down, and an item count shows how many
> match. A **grid/list toggle** (☰) switches the tile grid for a compact one-line-per-asset list (icon + type
> badge + name + read-only/modified indicators), better for scanning long folders. All of it reuses the existing
> mount-layer navigation and the same open/context-menu actions — pure view-model + XAML, no backend changes.
>
> Previously (**M42**): **rotate + scale gizmos, Local/World, snap.** The viewport's transform tooling is now
> a full move/rotate/scale gizmo. A **✥ Move / ⟳ Rotate / ⇲ Scale** toggle group in the viewport HUD switches the
> gizmo: Move keeps the axis arrows (drag to translate), Rotate draws three colour-coded rings (grab one, drag
> horizontally to spin the mesh about that axis), and Scale draws axis arms with box handles (drag a handle out
> or in to scale that axis). A **World/Local** toggle points the gizmo axes along world space or the mesh's own
> rotation, and a **⊞ Snap** toggle quantises to 100u / 15° / 0.25× increments. Every drag is one undo step
> (reusing the M28/M29 begin/end-drag + command stack) and writes back through the existing byte-patch persistence
> — no new transform math, just the interactive handles on top of the M26 rotate/scale backend. The gizmo
> geometry (rings, box handles) was render-verified headless in all three modes.
>
> Previously (**M41**): **placed prop meshes render on the map.** The M38 animated props are no longer just
> markers — the actual creatures and structures now appear in the world. A prop's `skin` path resolves to its
> skin `.bin` → `simpleSkin` SKN mesh + per-submesh materials (`SkinMeshExtractor` + the existing champion
> mesh/material loaders), each unique mesh is decoded once and instanced at every placement. In the **Animated
> Props** outliner folder, hit **🧊 Meshes** and Baron appears in his pit, all six dragons + elder in the dragon
> pit, every jungle camp (razorbeaks, wolves, gromp, krugs, blue/red buffs, scuttle) in its clearing, and the
> turrets, inhibitors and Nexus crystals along the lanes and in the bases — textured, lit, depth-tested against
> the terrain, with per-instance winding so mirrored (red-side) placements cull correctly under the -X mirror.
> Decoding runs off the UI thread; unresolved skins fall back to their markers. Verified headless on Old SR:
> 89 placements → 23 unique meshes decoded + textured → 61 instances rendered at exactly the right spots.
> (Static bind-pose — per-prop idle animation is a later step, and M40 added a full Preferences window for
> rebindable keybinds + camera feel.)
>
> Previously (**M39**): **PROJECT Kalista UI redesign.** ReyEngine now looks like the modern editor it acts
...
| **M43 ✅** | **Content Browser polish** — clickable breadcrumb path (⌂ home + per-folder buttons via the existing `Breadcrumbs`/`NavigateCrumbCommand`, jump up N levels in one click); type-filter dropdown (All/Maps/Bins/Textures/Meshes/Skeletons/Animations/Materials/Shaders/Audio → `AssetNodeViewModel.Kind`, folders always kept so you can still navigate) + live item count; grid/list view toggle (☰) with a compact one-row-per-asset list (icon + type badge + name + read-only/modified). VM adds `TypeFilter`/`ListView`/`ItemCount` + `ToggleView`; reuses the same open/context-menu actions — XAML + VM only |
| **M42 ✅** | **Transform gizmo modes** — the translate-only viewport gizmo becomes full move/rotate/scale. HUD toggle group (✥ Move / ⟳ Rotate / ⇲ Scale) + World/Local + ⊞ Snap. Renderer `SetGizmo` gains a mode + per-axis directions: move arrows, rotate rings (circle line-loops in each axis plane), scale arms with box-handle tips. Hit-testing per mode (axis arms for move/scale, nearest projected ring for rotate); drag applies translate (frozen-axis param), scale (axis-distance ratio → per-axis scale) or rotate (horizontal drag → degrees about the axis), all snapped when enabled and committed as one undo step via the existing begin/end-drag + `MeshTransformCommand`. Local space rotates the gizmo axes by the mesh's own rotation. Reuses the M26 `RotateMesh`/`ScaleMesh` backend — no new transform math. Gizmo geometry render-verified headless in all 3 modes |
| **M41 ✅** | **Placed prop meshes** — the M38 animated props render as real meshes, not markers. `SkinMeshExtractor` reads `simpleSkin`/`skeleton`/default `texture` from a creature skin `.bin`; each unique skin is decoded once (SknDecoder + `ChampionMaterialResolver` per-submesh diffuse) and instanced per placement. Renderer gains a prop-instance system (`RegisterPropGeometry`/`AddPropInstance`/`ClearProps`): own VAO/VBO/EBO, basic-lit diffuse pass reusing the mesh shader, per-instance front-face winding from the transform determinant (mirrored red-side placements cull right under the -X mirror), depth-tested vs terrain, prop textures freed on map switch. App: **🧊 Meshes** toggle in the Animated Props folder decodes off-thread + shows Baron/dragons/camps/turrets/inhibitors/Nexus at their spots; unresolved skins keep their markers. Verified headless: 89 placements → 23 meshes → 61 textured instances at correct positions |
| **M40 ✅** | **Preferences window** — persisted `EditorSettings` (`%AppData%/ReyEngine/settings.json`): rebindable viewport fly/focus keybinds (click a pill, press a key), camera sensitivity multipliers (look/orbit/pan/zoom) + invert-Y + base fly speed, default cull toggle; Settings window (Controls/Camera/Viewport tabs, key-capture via tunnel KeyDown, Reset/Cancel/Save) opened from Edit ▸ Preferences / Help / title-bar gear / Ctrl+,; applied live to the viewport + input handlers, round-trip verified |
