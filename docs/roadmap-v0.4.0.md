...
| # | Item | Size | Notes |
|---|------|------|-------|
| 2b | DX11 live edits: paint and topology | **S–M, now scoped from code** | Answered without the manual test. **Paint is a TEXTURE path, not a vertex path**: a stroke calls `PushTextureRegion` / `RebuildTextureMips`, host hooks wired in `MainWindow.axaml.cs:358-360` to `Viewport.QueueTextureUpdate` and `Viewport.RequestMipRebuild` — GL only. It raises neither `MeshVerticesRevision` nor `MapGeneration`, which is why no existing DX11 signal carries it. The renderer already has `SetTexture(material, reflectedName, key, rgba, w, h)` (whole-texture re-upload) and uses `UpdateSubresource` elsewhere, so the work is a second subscriber on those two hooks plus a sub-region entry point if whole-texture upload proves too slow mid-stroke — measure before optimising. **Topology (Add Mesh / delete) is still unverified**: no `MapGeneration++` found in the Add Mesh completion path, but that search was not exhaustive. Confirm before building; if it is absent, raising `MapGeneration` is likely the whole fix, since that already triggers a full DX11 rebuild. |
