...
| # | Item | Size | Notes |
|---|------|------|-------|
| 2b | DX11 live edits: **topology only** | S — likely already works, needs a 10-second check | Traced: **Add Mesh never touches the mapgeo**, which is why no `MapGeneration++` exists in its path and why looking for one was the wrong question. `ExecuteAddMeshPlanAsync` STAGES meshes into `MapContent.AddedMeshes` and calls `PublishAddedMeshPreview()`, which combines them with props onto the **prop-instance channel** — they only enter the mapgeo on Save Map Edits. D3D11 already renders props (M295, `D3D11MapProps`), so added meshes plausibly appear there already. **Unconfirmed link:** whether the D3D11 prop driver reads the same combined list `PublishAddedMeshPreview` writes. **Check before coding:** with D3D11 on, Add Mesh and see if it shows. If yes, 2b is done. If no, feed the D3D11 prop driver the combined list rather than rebuilding the scene. Delete/removal is a separate path and was not traced. |
