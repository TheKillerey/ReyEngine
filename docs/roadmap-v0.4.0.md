...
| # | Item | Size | Notes |
|---|------|------|-------|
| 3b | DX11: port the skybox | **M** — scoped, ready to build | `SkyboxRenderer` is 292 lines with **three modes** (cubemap, equirect, sky-dome mesh) and **two GLSL shader pairs** (`DirVert/DirFrag` for cube+equirect, `MeshVert/MeshFrag` for the dome), so a port is two HLSL pipelines, not one. **Build it on the overlay pipeline's pattern** (`ShaderPreviewRenderer`: own HLSL string, lazy `EnsureOverlay()`, own VS/PS/layout/CB, own depth states) — that is the closest existing model. Reusable already: cube SRV creation exists (`MakeTexture(..., resourceDimension: 9)`, used for `_whiteCube`) but must be extended from 1 face to 6; `_overlayDepthNoTest` exists if the sky draws first with depth test off. **Unmeasured, settle before coding:** GL's exact sky depth state (test off and draw first, vs LEQUAL with depth-write off and draw last) — read `SkyboxRenderer.Render`, do not assume. Host side also needs the skybox asset fed to the DX11 surface; the GL side gets it from the M122 catalogue. |
...
| 7 | DX11: soft particles + beam/trail emitters | **M** — real, and precisely located | Confirmed real, not stale like most of this list: `D3D11MapParticles` **deliberately skips** them at one line — `if (def.Beam is not null || def.Trail is not null) { SkippedBeamTrailEmitters++; continue; }` — and already reports the count to the user with the reason, *"ribbon geometry, not billboards"*. That is the whole gap and the whole difficulty: the map-particle path builds **billboards** into one dynamic vertex buffer, while a beam or trail needs a **ribbon strip built from particle history**, which is a different vertex-generation shape rather than a different shader. The same file skips mesh-primitive emitters for the sibling reason, so a ribbon path may be worth designing alongside that. **Soft particles are a separate, smaller sub-item** and were NOT traced — check whether the D3D11 path has any scene-depth capture before assuming it needs one (the M282 distortion work already captures a scene copy, which may be reusable). |
