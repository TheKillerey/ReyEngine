...
| # | Item | Size | Notes |
|---|------|------|-------|
| 2b | DX11 live edits: **topology only** | S, unverified | **Paint is done** — M360 (`497f18b`) routes strokes to D3D11 and M361 (`134f6b7`) draws the brush ring there, both verified by the user on a real map. Paint turned out to be a TEXTURE path, not a vertex one: it raises neither `MeshVerticesRevision` nor `MapGeneration`, so no existing signal carried it. Remaining: **topology** (Add Mesh / delete). Still unverified — no `MapGeneration++` was found in the Add Mesh completion path, but that search was not exhaustive. If it is genuinely absent, raising `MapGeneration` is likely the whole fix, since that already triggers a full D3D11 rebuild. |
