...
| 7b | ~~DX11: beam/trail ribbon emitters~~ | **Done** (M364). Built as a dedicated ribbon pipeline, NOT the M283 mesh channel the scope named: that channel is instanced pos+uv geometry drawn per particle through Riot's shader, while a ribbon is one world-space strip with per-vertex colour. GL settles it - GL does not use Riot's shader for ribbons either. **That choice dissolved all three "open questions" at once:** the varying vertex count is handled by growing the buffer exactly as GL does (no degenerate-tail convention needed), UVs ride in the interleaved vertex, and beams already resolved endpoints without a target via the authored offset (Riot's `_Long_FakeDir` pattern). Two of the three were artifacts of the wrong vehicle, not real problems. Sharing went deeper than planned for a hard reason: trail history is `internal` to ReyEngine.Rendering, so the assembly step had to move there too - `BuildTrailRibbon`/`BuildBeamRibbon` are now shared and GL was refactored onto them, so the two renderers cannot drift. |
| 5 | ~~DX11: grass tint~~ | **Done** (M355, `d9b657b`). Was genuinely real - zero references to grass tint in any DX11 file. Bound from what the shader permutation actually declares (`GRASS_TINT_MAP_SharedTexture` + `TERRAIN_XFORM`) rather than ported from GL's single-texture approximation. Row was left in Core by oversight; the work shipped. |
...
**All core items are done.** What remains for the release is verification on real maps and the release mechanics, not implementation.
